...
int main(int argc, char **argv) {
	...
	if (my_rank == 0) {
		...
		// for-loop to get the pairs of most distinct test cases
		int pair2rm[2];
		for (int t_id = 0; t_id < noResets; t_id+=2)
		{
			...
			///////////////////////////////////////
			// INCLUDE IN t_prtz, REMOVE FROM ts //
			///////////////////////////////////////
		...
		int keyPos_i = floor((((noResets*(noResets-1))/2.0)/(num_proc-1))*(my_rank-1)); // inclusive    : [i
		int keyPos_f = trunc((((noResets*(noResets-1))/2.0)/(num_proc-1))*(my_rank));   // not inclusive: j)
		...
		while(!simPair.empty()) {
			...
			/////////////////////////////////////////////
			// Get the last most distinct using rbegin()
			/////////////////////////////////////////////
			std::multimap<double,std::pair<int,int>>::reverse_iterator rit = simPair.rbegin();

			///////////////////////////////////////////////
			// Decrements to the first-last most distinct
			///////////////////////////////////////////////
			...
			////////////////////////////////////////////////////
			// Removes all pairs with pair2rm[0] or pair2rm[1]
			////////////////////////////////////////////////////
